...
**CRITICAL**: Engineering task IDs generated by `scan_task_breakdown()` MUST include the
cycle number prefix (e.g., `c003-eng-05`). The `JobQueue` deduplicates by ID —
without the cycle prefix, tasks from a previous cycle are silently dropped as "already completed."
