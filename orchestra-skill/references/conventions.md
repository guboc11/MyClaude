...
**CRITICAL**: ALL task IDs MUST include the cycle number prefix. The `JobQueue` deduplicates
by ID — without the cycle prefix, tasks from a previous cycle are silently dropped as
"already completed."
- Engineering: `scan_task_breakdown(cycle_number)` → `c003-eng-05`
- Standard phases: `read_task_files(phase, cycle_number)` → `c003-analysis-01`
Both functions enforce the `c{NNN}-{phase}-{NN}` format.
