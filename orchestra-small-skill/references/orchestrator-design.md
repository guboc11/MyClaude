...
PHASES = [
    "analysis",
    "research",
    "research_gate",  # NEW: Director validates real-world references
    "planning",
...
# Single-step phases (task_breakdown, wrapup, research_gate) use one pass.
STANDARD_PHASES = {"analysis", "research", "planning", "review", "qa"}
TWO_STEP_PHASES = {"engineering"}
SINGLE_STEP_PHASES = {"task_breakdown", "wrapup", "research_gate"}
...
# research_gate is SINGLE step — Director only, validates research output.
...
class CycleManager:
    ...
    def advance_phase(self) -> str | None:
        ...
        - SINGLE_STEP_PHASES (task_breakdown, wrapup, research_gate) → "SINGLE"
    ...
    def advance_phase_step(self) -> bool:
        ...
        Single-step phases (task_breakdown, wrapup, research_gate): always returns True."""
    ...
    def get_pending_tasks(self) -> list[Task]:
        ...
        research_gate (SINGLE_STEP_PHASES):
          SINGLE → [director_task(mode="research_gate")]
              Director reads research/real-world-references.md
              Validates: min_references >= 3, all URLs accessible, no AI-fabricated content
              Pass: writes research_gate-complete.md → cycle continues to planning
              Fail: writes research_gate-retry.md with specific gaps, cycle_state.retry_count++
                    If retry_count >= config.research_gate.max_retries:
                      Notify Admin via .orchestra/requests/pending/REQ-research-gate-failed.md
                      Force advance to planning regardless (don't block indefinitely)
                    Else: cycle manager resets to "research" phase (research re-runs)
        ...
        engineering (TWO_STEP_PHASES — skips DEPT_HEAD_INITIAL):
          MEMBERS            → returns tasks pre-loaded by phase transition hook
              ...
              NOTE: ownership-registry.json is created here before Members spawn.
                    See "File Ownership Registry Hook" section below.
          DEPT_HEAD_COMPLETION → [dept_head_task(mode="completion")]
    ...
    def is_phase_complete(self) -> bool:
        ...
        For small phases (total_count < 3 — e.g. task_breakdown, wrapup, research_gate),
    ...
    def should_audit(self) -> bool:
        """True if cycle_number % audit.every_n_cycles == 0 (every 3 cycles)."""
    ...
    def start_new_cycle(self) -> None:
        ...
        - completed_tasks → {}
        - failed_tasks → {}
        - retry_count → 0  # Reset research_gate retry counter
...
class Job:
    id: str
    priority: Priority
    model: str             # "opus" / "sonnet" (mapped to real ID by agent_runner)
...
def build_agent_prompt(task: Task, config: Config) -> str:
    ...
    Structure:
      1. Onboarding file content (opus-onboarding.md / sonnet-onboarding.md)
      2. Persona file content (LEAD.md / MEMBER.md)
...
if consecutive_success[model] >= config.adaptive_concurrency.increase_after_n_success:
    max_concurrent = min(max_concurrent + 1, config.adaptive_concurrency.max)  # max: 15
...
if consecutive_429[model] >= config.adaptive_concurrency.decrease_after_n_429:
    max_concurrent = max(max_concurrent - 1, config.adaptive_concurrency.min)  # min: 3
    consecutive_429[model] = 0
...
class FileBus:
    ...
    def create_ownership_registry(self, cycle_number: int, eng_tasks: list[Task]) -> Path:
        """Build ownership-registry.json from all engineering task file ownership declarations.

        Called AFTER scan_task_breakdown() and BEFORE engineering MEMBERS step.

        Process:
          1. For each task file in eng_tasks, parse "## File Ownership Declaration" section
          2. Collect {file_path → task_id} mappings
          3. Detect conflicts: same file claimed by multiple tasks
          4. Auto-resolve conflicts: convert conflicting parallel tasks to sequential order
             (append to a "sequential_after" list in the registry)
          5. Write .orchestra/workspace/current-cycle/engineering/ownership-registry.json

        Registry format:
        {
          "cycle": 3,
          "created_at": "2026-03-18T23:30:00",
          "ownership": {
            "src/components/Foo.tsx": "c003-eng-01",
            "src/styles/foo.css":     "c003-eng-01"
          },
...
- OWNS: src/components/MyFeature.tsx
- OWNS: src/styles/my-feature.css
- OWNS: src/lib/my-feature-utils.ts
...
- .orchestra/workspace/current-cycle/engineering/ownership-registry.json
...
### research_gate Phase (between Research and Planning)

```
1. Orchestrator spawns Director (Opus) — SINGLE step
   - Prompt: "Validate research/real-world-references.md.
     Requirements:
       - At least 3 concrete references with accessible URLs
       - No fabricated or AI-hallucinated sources
       - References must be relevant to the feature being built
     If validation PASSES:
       Write research_gate-complete.md and mark approved=true.
     If validation FAILS:
       Write research_gate-retry.md with specific gaps found.
       List what types of references are missing."

2. Orchestrator reads Director output
   - If approved: advance to planning phase normally
   - If retry needed:
       cycle_state.retry_count += 1
       If retry_count >= config.research_gate.max_retries (3):
         Write .orchestra/requests/pending/REQ-research-gate-failed.md to notify Admin
         Force advance to planning (don't block cycle indefinitely)
       Else:
         Reset current_phase to "research"
         Research Department re-runs with retry context appended to their prompt
```
...
1. Orchestrator spawns Director (Opus)
   - Prompt: "Read planning/sprint-plan.md and planning/feature-spec.md.
     ...
     Format: task-eng-{NN}.md with:
       - File Ownership Declaration (files this task will modify)
...
### File Ownership Registry Hook (between task_breakdown and engineering)

```
After task_breakdown completes and BEFORE engineering MEMBERS step:

1. file_bus.scan_task_breakdown() → list[Task]
2. file_bus.create_ownership_registry(cycle_number, eng_tasks)
   - Parses each task file's "## File Ownership Declaration" section
   - Builds ownership map: {file_path → task_id}
   - Detects conflicts (same file in multiple tasks)
   - Auto-resolves: conflicting tasks are added to sequential_groups[]
   - Writes ownership-registry.json
3. cycle_mgr.load_phase_tasks("engineering", eng_tasks)

Engineering Members receive ownership-registry.json as a context file.
Members must NOT modify files not listed in their ownership declaration.
```
...
### Audit Phase (every 3 cycles, runs in parallel)

```python
async def launch_audit(config: Config, cycle_number: int, file_bus: FileBus):  # [FIX BUG-3]
    """
    Spawn the audit department as a parallel async task (every 3 cycles).
    Runs alongside the main cycle — does not block progression.

    Scale (small): Dept Head (Opus) + Sonnet x5
    Scope: security, AI-smell, code quality, bugs, deps

    1. Spawn Audit Department Head (Opus)
       - Head reviews entire codebase and creates audit task assignments
    2. Spawn Members (Sonnet x5) per task file
    3. Dept Head writes final audit-cycle-{N}.md
    4. Save to .orchestra/reports/audit/
    """
    audit_queue = JobQueue(max_concurrent=5)  # Separate queue — small audit team
    ...
    def _audit_head_prompt() -> str:
        return (
            ...
            f"   code quality issues, bugs/edge cases, dependency vulnerabilities.\n"
            f"3. Break your audit into up to 5 tasks. Write each as a separate file to:\n"
    ...
    def _audit_completion_prompt() -> str:
        return (
            f"You are the Audit Department Head (Opus). All Cycle {cycle_number} audit members are done.\n\n"
            f"1. Read all member outputs from .orchestra/workspace/current-cycle/audit/\n"
    ...
    # Step 1: Spawn Audit Dept Head to create task assignments
    head_job = Job(
    ...
    # Step 2: Read task files written by Dept Head, spawn Members (Sonnet).
    audit_tasks = file_bus.read_task_files("audit", cycle_number)
    if not audit_tasks:
        log.warning(f"Audit cycle {cycle_number}: Dept Head wrote no task files. Skipping member phase.")
    else:
        for task in audit_tasks:
            job = create_job(task, config)
...
async def main():
    ...
    queue = JobQueue(max_concurrent=config.orchestrator.adaptive_concurrency.start)  # Start at 8
    ...
    consecutive_auth_failures = 0     # Tracks 401s — 3 consecutive → shutdown
    ...
    while True:
        ...
        # Single-step phases (task_breakdown, wrapup, research_gate): one pass then done.
        while True:
            ...
            # research_gate: check if Director approved or requested retry
            if cycle_mgr.current_phase == "research_gate":
                retry_file = ".orchestra/workspace/current-cycle/research_gate/research_gate-retry.md"
                if Path(retry_file).exists():
                    cycle_mgr.state.retry_count = getattr(cycle_mgr.state, "retry_count", 0) + 1
                    if cycle_mgr.state.retry_count >= config.research_gate.max_retries:
                        log.warning(f"research_gate: max retries ({config.research_gate.max_retries}) reached. Notifying Admin and forcing advance.")
                        write_admin_request("research_gate_failed", cycle_mgr.cycle_number)
                        # Fall through — advance_phase_step will proceed normally
                    else:
                        log.info(f"research_gate: retry {cycle_mgr.state.retry_count}. Resetting to research phase.")
                        cycle_mgr.current_phase = "research"
                        cycle_mgr.current_phase_step = "DEPT_HEAD_INITIAL"
                        cycle_mgr.save_state()
                        break  # Re-enter inner loop at research phase
            ...
            if phase_done:
                if not cycle_mgr.is_phase_complete():
                    ...
                    cycle_mgr.reset_to_members_step()
                    continue
                break

        # Check if audit should run (parallel, every 3 cycles)
        ...
        # Phase transition hook: preload MEMBERS tasks for standard phases
        ...
        # Phase transition hook: load task_breakdown output + create ownership registry
        if next_phase == "engineering":
            ...
            if not eng_tasks:
                ...
                # Create ownership registry BEFORE members spawn
                registry_path = file_bus.create_ownership_registry(cycle_mgr.cycle_number, eng_tasks)
                log.info(f"Ownership registry created: {registry_path}")
        ...
        if next_phase is None:
            ...
            if build_result.exit_code == 0:
                summary = cycle_mgr.get_cycle_summary()
                run("git add -A")
            ...
            if cycle_mgr.graceful_shutdown_requested:  # [FIX BUG-9]
                if audit_task and not audit_task.done():
                    ...
                    try:
                        await asyncio.wait_for(audit_task, timeout=1800)
                    except asyncio.TimeoutError:
...
if result.exit_code == 0:
    ...
    cycle_mgr.save_task_state(job.id)  # [FIX BUG-10] Save immediately
...
# Agent task failed (non-429)
    ...
    if job.retry_count < 1:   # [FIX BUG-2]
        job.retry_count += 1
        await queue.submit(job)
    else:
        cycle_mgr.mark_task_failed(job.id, result.stderr[:1000])  # [FIX BUG-1]
...
No Haiku. Both Opus and Sonnet have full tool access.
