...
> **원본은 항상 md다.** pdf는 파생 산출물 — `.claude/mcp-md-convert/{날짜}-{주제}/`에 쌓이고 git 추적 제외다.
> 산출 루트는 도구 이름을 따른다 — 형식별 폴더(.claude/pdf 등)는 형식이 늘 때마다 폴더가 늘어나 금지. 형식은 확장자가 말한다.
> 설계 경위: `_ARCHIVED/_PLAN/2026-08-01-md-convert-mcp/PLAN.md`
...
- `convert_pdf { files: [경로...], topic: "주제", landscape?: false }`
  ...
  - `topic` — 산출 폴더의 주제부. `.claude/mcp-md-convert/{오늘}-{topic}/`이 자동 생성된다 (kebab-case 권장)
...
의존성(marked·github-markdown-css)이 없으면 도구가 아래 한 줄을 안내하고 멈춘다 — 그대로 실행하면 된다 (자동 설치는 하지 않는다):
...
양식은 두 장이 겹쳐 만든다. 순서대로 붙으므로 뒤가 이긴다.

1. **`github-markdown-css` (light 판)** — 본 양식. 글자·색·표·코드·인용이 전부 여기서 온다.
   마크다운 미리보기와 같게 보이는 것이 목적이라 **양식을 직접 쓰지 않는다.**
   light 판을 쓰는 이유는 통합판의 `prefers-color-scheme` 때문에 인쇄가 다크로 뒤집힐 수 있어서다.
2. **`style.css`** — 종이에만 필요한 것만 덮어쓴다. 화면에 있는 것(가로 스크롤)이 종이에 없어서
   생기는 문제와 페이지 넘김이 전부다.

- 본 양식이 마음에 안 들면 `style.css`에 덮어쓰기를 더한다. github-markdown-css 파일은 고치지 않는다(설치본이라 갱신 시 날아간다).
- A4 기준은 `_CRAFT_GUIDE/PRINT_CONVENTIONS.md` §1을 따르되, 여백 0은 전면 SVG 다이어그램용이라 글 문서인 여기는 본문 여백(14mm/15mm)을 둔다.
- 표는 GitHub 기본이 `width: max-content`라 그대로 두면 종이 밖으로 잘려 나간다 — `width: 100%`로 덮어쓴다.
- 한글은 낱말 경계가 없어 기본값이면 낱말 가운데서 끊긴다 — 표 칸에 `word-break: keep-all`.
- 긴 표가 여러 페이지에 걸치는 건 정상. 행 단위 잘림만 막고(`tr { break-inside: avoid }`) 헤더는 페이지마다 반복한다.
...
| `style.css` | 인쇄 덮어쓰기 (본 양식은 github-markdown-css) |
| `package.json` | 의존성 격리 (marked·github-markdown-css) — 레포 앱 의존성과 무관 |
